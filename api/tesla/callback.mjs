...
export default async function callback(request, response) {
	try {
		...
		if (!clientId || !clientSecret || !domain || !code || !state || state !== expectedState || signature !== expectedSignature) throw new Error("Tesla-login kunne ikke valideres");

		const tokenResponse = await fetch(teslaAuthUrl, {
			method: "POST",
			headers: { "Content-Type": "application/x-www-form-urlencoded", Accept: "application/json" },
			body: new URLSearchParams({ grant_type: "authorization_code", client_id: clientId, client_secret: clientSecret, code, redirect_uri: redirectUri, audience: teslaApiUrl }),
		});
